...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      let blur = 0;
      if (blurAmount) {
        blur = Math.max(0, i * blurAmount * scaleProgress);
      }

      const translateY = Math.max(0, scrollY - triggerStart);
  ...
  }, [itemScale, itemStackDistance, stackPosition, scaleEndPosition, baseScale, rotationAmount, blurAmount, calculateProgress, parsePercentage]);
